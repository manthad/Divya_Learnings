...
so i write dockerfile, docker compose files, terraform if necessary or github actions yml files where i write on push or pull then the only part developer does is just push the code using git push then automatcially they go and check the actions workflow whether it is pass or fail. 
