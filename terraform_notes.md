...
### 3. Variables: (var.tf)  
#### input.tfvars
- here directly we provide input data 

    ```bash
    instance_type = "t2.micro"
    ```
#### variables.tf  
- here we just define what value and its data type to which we are inputing values in inputvars
    ```bash
...
variable "instance_type" {
description = "Type of instance"       # here in main file we mention as 
instance_type = var.instance_type # then all the data is taken from here
default     = "t2.micro"
}
```
### 4. Outputs: 
...
|Command     | Use|
|----------|----------|
|terraform init|     initialize the terraform in the directory. it will download the providers. this is like setting all the providers, plugins etc ready for our work to start like arranging all the things prior.
| terraform plan|    shows plan of resources like what is created, deleted or modifed
| terraform apply|     create the resources. it will ask for confirmation before creating the resources.
| terraform destroy| destroy the resources. it will ask for confirmation before destroying the resources.
| terraform validate|  validate the terraform files, will check for any syntax errors.
| terraform show|     show the details of the resources created.
| terraform state|     manage the state file. it has many sub-commands like list, show, rm etc.
|terraform import|    import the existing resources into terraform.
...
### Create Backup:
To create backup in the s3 bucket use 
```bash
...
- now the range is 11111111.11111111.11111111.00000000 to 11111111.11111111.11111111. 10000000 or 

    last number is as as exisitng ip address
...
private key (.pem file which is the key) and both should match to login
        ...
        ssh -i divya-key.pem ubuntu@12.34.222.39  
- here .pem is private and public key will tally with the public ip ofthe instance and it should match
...
- In general we create key pair with command below and it will autoamticlaly generates public an private key. 
    - Private is used for ssh into instance
    - Public for validating that private matahces to this or not 
...
connection is like ssh and ubuntu and .pub is divyamantha and 
...
If we want to create the same resources in dev, uat and test env 
